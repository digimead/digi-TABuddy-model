...
class ProtobufSerializationSpec_j1 extends FunSpec with ShouldMatchers with TestHelperLogging {
  ...
  describe("A ProtobufSerialization") {
    ...
    it("should provide serialization mechanism for Element") {
      config =>
        ...
        save.eChildren.head.asInstanceOf[Record[Record.Stash]].description should be("456")
    ...
    it("should filter elements on save/load") {
      config =>
        ...
        def fFilterSave(element: Element.Generic): Option[Element.Generic] = {
          ...
          element match {
            case note: Note.Generic if note.eId == 'note1 =>
              // alter note1
              note.description = "save_filter_added"
              Some(note)
        ...
        def fFilterLoad(element: Element.Generic): Option[Element.Generic] = {
          ...
          element match {
            case note: Note.Generic if note.eId == 'note2 =>
              // alter note2
              note.description = "load_filter_added"
              Some(note)
            case element =>
              // pass all other
              Some(element)
        ...
        deserializedNote1.description should be("save_filter_added")
        val deserializedNote2 = deserializedModel | NoteLocation('note2)
        deserializedNote2.description should be("load_filter_added")
    }
  }
}
