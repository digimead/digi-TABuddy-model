...
class YAMLSerialization extends Serialization[String] {
  ...
  def acquire[A <: Element[B], B <: Stash](loadElement: () => Option[String],
    ...
    while (data.nonEmpty) {
      try {
        YAMLSerialization.YAML.from(data.get) match {
          case Some(element) =>
  ...
  @tailrec
  private def freezeWorker(saveElement: (Element.Generic, String) => Unit,
    ...
    val saved = elements.map { element =>
      ...
      filter(serialized) match {
        case Some(filtered) =>
          saveElement(element, YAMLSerialization.YAML.to(filtered))
...
object YAMLSerialization extends Loggable {
  object YAML {
    ...
    def to(value: Element.Generic): String = {
    ...
    object ElementConstructor extends Constructor(classOf[Element.Generic]) {
      ...
      val tagProperty = new Tag(classOf[YAML.Property])
      ...
      class AxisConstruct extends CustomConstruct {
        def constructCustom(map: mutable.HashMap[String, Node]): AnyRef = try {
          ...
          case e: Throwable =>
            log.error("unable to unpack axis: " + e)
            null
        }
      }
      class ContextConstruct extends CustomConstruct {
        def constructCustom(map: mutable.HashMap[String, Node]): AnyRef = try {
          ...
          case e: Throwable =>
            log.error("unable to unpack context: " + e)
            null
        }
      }
      ...
      class PropertyConstruct extends CustomConstruct {
        def constructCustom(map: mutable.HashMap[String, Node]): AnyRef = try {
          ...
          } yield YAML.Property(context, typeSymbol, data, id, static)
          if (property.isEmpty)
            log.error("unable to unpack property")
          property.getOrElse(null)
        } catch {
          case e: Throwable =>
            log.error("unable to unpack property: " + e)
            null
        }
      }
      class ReferenceConstruct extends CustomConstruct {
        def constructCustom(map: mutable.HashMap[String, Node]): AnyRef = try {
          ...
          case e: Throwable =>
            log.error("unable to unpack reference: " + e)
            null
        }
      }
      class StashConstruct extends CustomConstruct {
          ...
          val stash = for {
            ...
            property <- map.get("property").flatMap {
              case snode: SequenceNode =>
                ...
                Option(ElementConstructor.constructObject(snode).asInstanceOf[java.util.List[YAML.Property]])
            }
        ...
        protected def unpackProperties(list: java.util.List[YAML.Property]): org.digimead.tabuddy.model.element.Stash.Data = {
    ...
    class ElementRepresenter extends Representer {
      ...
      multiRepresenters.put(classOf[YAML.Property], new PropertyRepresent)
      ...
      class PropertyRepresent extends Represent {
        override def representData(data: AnyRef): Node = {
          val property = data.asInstanceOf[YAML.Property]
          val map = new java.util.HashMap[String, AnyRef]()
      ...
      class StashRepresent extends Represent {
        override def representData(data: AnyRef): Node = {
          ...
          var properties = Seq[YAML.Property]()
            ...
            stash.property(valueID).foreach {
              case (typeSymbol, value) if DSLType.symbols(typeSymbol) =>
                DSLType.convertToString(typeSymbol, value.get) match {
                  case Some(valueData) =>
                    properties = properties :+ YAML.Property(value.context, typeSymbol.name, valueData, valueID.name, value.isInstanceOf[Value.Static[_]])
                  case None =>
